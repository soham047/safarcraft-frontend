...
export default function Navbar() {
  ...
  return (
    <nav className="relative z-50">
      ...
      <div className="bg-gray-900/70 backdrop-blur-md shadow-md text-white px-6 py-4 flex justify-between items-center">
        <Link to='/'>
          <img
            ...
            className={`h-10 ${isOpen ? 'hidden' : ''} transition-all duration-300`}
          />
        </Link>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="hamburger-menu animate__animated animate__fadeInRight">
          ...
          <div>
            <button
              ...
              className="w-full text-left py-2 px-4 font-medium bg-gray-900 border-b border-gray-700"
            >
              Services {isServiceOpen ? "▲" : "▼"}
            </button>
            {isServiceOpen && (
              <div className="pl-4 text-sm">
                <Link to="/cost" onClick={() => setIsOpen(false)} className="block py-1">Cost Calculator</Link>
                <Link to="/itinerary" onClick={() => setIsOpen(false)} className="block py-1">Itinerary Planner</Link>
                <Link to="/similar" onClick={() => setIsOpen(false)} className="block py-1">Similar Place Suggestor</Link>
                <Link to="/packing" onClick={() => setIsOpen(false)} className="block py-1">Packing List Generator</Link>
